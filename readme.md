...
* `<<`/`>>` - The right argument is appended to the right argument, which must be an object. The arrows point towards what end the element is being appended to. For example, ```2 1 ``List 3 <<```, will give ```` 3 2 1``` ````, because the `3` was appended to the end of the object.
* `<>` - The left and right objects are merged. It throws if their tags are different. For example, ```2 1 ``List 4 3 ``List <>``` will give `````4 3 2 1 ````List`````.
* `&` - This runs the function on the top of the stack in a safe environment, on a *copy* of the current stack. If the function throws, `&` returns an empty `None` object. Otherwise is removes the items that were touched, and pack them in a `Some` object. For example, `3 2 1 'swap &` will leave the stack as ``3 2 1 (2, 1) `Some`` because `swap` only touched `1` and `2`, but left `3` alone. The numbers in `(2, 1)` were effectively swapped as the front represents the top of the stack (The parenthesis notation will be explained after the built-ins). On the other hand, `3 2 1 'Pair? &` will leave the stack as `3 2 1 None` since `Pair?` throws on `1`.
